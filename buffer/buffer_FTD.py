...
def train(pid, args, channel, num_classes, im_size, trainloader, testloader, save_dir):
    ...
    for it in range(20):
        ...
        if len(trajectories) == args.save_interval:
            ...
            #     n += 1
            if not os.path.exists(os.path.join(save_dir, "replay_buffer_{}.pt".format(pid))):
                ...
                print("Saving {}".format(os.path.join(save_dir, "replay_buffer_{}.pt".format(pid + 1))))
                torch.save(trajectories, os.path.join(save_dir, "replay_buffer_{}.pt".format(pid + 1)))
...
def main(args):
    ...
    process_list = []
    for pid in range(args.num_experts // 20):
        p = mp.Process(target=train,args=(2 * pid, args, channel, num_classes, im_size, trainloader, testloader, save_dir))
        p.start()
        process_list.append(p)
        
    for i in process_list:
        p.join()
    # train(0, args, channel, num_classes, im_size, trainloader, testloader, save_dir)
